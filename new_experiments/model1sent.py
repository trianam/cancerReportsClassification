...
class MyModel(nn.Module):
    def __init__(self, conf):
        ...
        if conf.aggregationType == "softmax":
            self.phraseAttentionLayer = nn.Linear(nextFeaturesDim, conf.attentionDim)
            self.phraseContextVector = nn.Parameter(torch.randn(conf.attentionDim))
            self.phraseContextVector.requires_grad=True
        
        if conf.rnnType == 'LSTM':
            self.docLayer = nn.LSTM(nextFeaturesDim, conf.docFeaturesDim, conf.docFeaturesLayers, bidirectional=True, batch_first=True)
        elif conf.rnnType == 'GRU':
            self.docLayer = nn.GRU(nextFeaturesDim, conf.docFeaturesDim, conf.docFeaturesLayers, bidirectional=True, batch_first=True)
        
        if self.conf.bidirectionalMerge is None:
            nextFeaturesDim = 2 * conf.docFeaturesDim
        else:
            nextFeaturesDim = conf.docFeaturesDim

        if conf.aggregationType == "softmax":
            self.docAttentionLayer = nn.Linear(nextFeaturesDim, conf.attentionDim)
            self.docContextVector = nn.Parameter(torch.randn(conf.attentionDim))
            self.docContextVector.requires_grad=True
    ...
    def forward(self, x, batchIndex=None, getAttention=False):
        ...
        if getAttention and (self.conf.aggregationType == "max" or self.conf.getAttentionType == "features"):
        ...
        if self.conf.aggregationType == "max":
            x = torch.max(x, 1).values

        elif self.conf.aggregationType == "softmax":
            a = self.phraseAttentionLayer(x)
            a = torch.tanh(a)
            a = torch.matmul(a, self.phraseContextVector)
            a = torch.softmax(a,1)
        ...
        #doc level
        x = x.view(-1, self.conf.numPhrases, nextFeaturesDim)
        seqLen = x.shape[1]
        ...
        if self.conf.bidirectionalMerge is None:
            nextFeaturesDim = 2*self.conf.docFeaturesDim
        else:
            x = x.view(-1, seqLen, 2, self.conf.docFeaturesDim)
            if self.conf.bidirectionalMerge == 'avg':
                x = (x[:,:,0] + x[:,:,1]) / 2.
            elif self.conf.bidirectionalMerge == 'max':
                x = torch.max(x, 2).values  
            
            nextFeaturesDim = self.conf.docFeaturesDim
        ...
        if getAttention and (self.conf.aggregationType == "max" or self.conf.getAttentionType == "features"):
            ...
            #statusPhrase = statusPhrase.view(-1, self.conf.numFields, self.conf.numPhrases, self.conf.docLen, nextFeaturesDim)
        ...
        if self.conf.aggregationType == "max":
            ...
            a = self.docAttentionLayer(x)
            a = torch.tanh(a)
            a = torch.matmul(a, self.docContextVector)
            a = torch.softmax(a,1)
        ...
        if getAttention:
            return x, (statusPhrase, statusDoc)
        else:
            return x
