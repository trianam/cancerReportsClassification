...
fileSaveDistances = "editDistances-{}.npz".format(sys.argv[1])
...
for i in range(splits[int(sys.argv[1])], splits[int(sys.argv[1])+1]):
    for j in range(i+1, numDocs):
        if j % 1000 == 0:
            print("{}/{} - {}/{}          ".format(i, splits[int(sys.argv[1])+1]-1, j, numDocs), end='\r', flush=True)
...
print("{}/{} - {}/{}        ".format(i, splits[int(sys.argv[1])+1]-1, j, numDocs), flush=True)

corpus['distance'] = dist

# # write

np.savez(fileSaveDistances, text=corpus['text'], sede1=corpus['sede1'], morfo1=corpus['morfo1'], distance=corpus['distance'])
