...
class MyModel(nn.Module):
    ...
    def forward(self, x, batchIndex=None, getAttention=False):
        ...
        if self.conf.masking:
            mask1 = mask.view(-1, self.conf.phraseLen, 1).expand(-1, -1, self.conf.phraseFeaturesDim*2)
            att = att * mask1
            #att = att - (1-mask1)
        ...
        for l in self.afterPhraseLayers[:-1]:
            att = l(att)
            att = torch.relu(att)
        att = self.afterPhraseLayers[-1](att)
        att = torch.tanh(att)
 
        if self.conf.masking:
            mask2 = mask.view(-1, self.conf.phraseLen, 1)
            att = att * mask2
            #att = att - (1-mask2)
        
        if getAttention:
            status = att.clone().detach()
            status = status.view(-1, self.conf.numFields, self.conf.numPhrases, self.conf.phraseLen)
        ...
        if self.conf.masking:
            mask1 = mask.view(-1, self.conf.numPhrases, 1).expand(-1, -1, self.conf.fieldFeaturesDim*2)
            att = att * mask1
            #att = att - (1-mask1)
        ...
        for l in self.afterFieldLayers[:-1]:
            att = l(att)
            att = torch.relu(att)
        att = self.afterFieldLayers[-1](att)
        att = torch.tanh(att)
       
        if self.conf.masking:
            mask2 = mask.view(-1, self.conf.numPhrases, 1)
            att = att * mask2
            #att = att - (1-mask2)
        ...
        if self.conf.masking:
            mask1 = mask.view(-1, self.conf.numFields, 1).expand(-1, -1, self.conf.docFeaturesDim*2)
            att = att * mask1
            #att = att - (1-mask1)
        ...
        for l in self.afterDocLayers[:-1]:
            att = l(att)
            att = torch.relu(att)
        att = self.afterDocLayers[-1](att)
        att = torch.tanh(att)
        
        if self.conf.masking:
            mask2 = mask.view(-1, self.conf.numFields, 1)
            att = att * mask2
            #att = att - (1-mask2)
