...
class MyModel(nn.Module):
    def __init__(self, conf):
        ...
        if conf.rnnType == 'LSTM':
            ...
            self.sortSeqLayer = nn.LSTM(conf.seqFeaturesDim, conf.sortSeqFeaturesDim, conf.sortSeqFeaturesLayers, bidirectional=False, batch_first=True)
            ...
            self.sortSeqLayer = nn.GRU(conf.seqFeaturesDim, conf.sortSeqFeaturesDim, conf.sortSeqFeaturesLayers, bidirectional=False, batch_first=True)
    ...
    def forward(self, x, batchIndex=None, getAttention=False):
        if not (self.conf.bistep is None) and not (batchIndex is None):
