...
conf = configurationsMulticlass.configTMbaseMultiBest
...
yppT = clf.decision_function(tfidfTest)
...
yyT = np.zeros((len(yT), len(values)))
ypppT = []
for i in range(len(yppT)):
    ypppT.append([yppT[i][j] for j in [np.argwhere(clf.classes_ == c)[0][0] for c in range(len(values))]])
    yyT[i][yT[i]] = 1.

ypppT = np.array(ypppT)

pickle.dump({'y':yyT, 'yp':ypppT}, open("predictionsSVM-Amorpho.p", 'wb'))
