...
class MyLSTM:
    _filesFolder = "./filesFolds-LSTMPROOF"
    _memmapFolder = "./memmapFolds-LSTMPROOF"
    _tmpFolder = "./tmpFolds-LSTMPROOF"
    ...
    def __init__(self, fold=0, tasks=['sede1', 'sede12', 'morfo1', 'morfo2'], corpusFolder="corpusLSTM_ICDO3", foldsFolder="folds10", fileVectors="vectors.txt", phraseLen = 200, hiddenNodes=400, dropout=0.3, patience=4, batchSize=65, epochs=100):
        self._tasks = tasks
        ...
        modelsFolder = self._filesFolder+"/modelsLSTM/"+str(fold)
    ...
    def extractData(self):
        ...
        for task in self._tasks:
            ...
            self.XTrain[task] = np.memmap(self._fileMemmapXTrain[task], mode='w+', shape=(len(textTrain), self._phraseLen, self._vecLen), dtype=np.float)
            self.XTest[task] = np.memmap(self._fileMemmapXTest[task], mode='w+', shape=(len(textTest), self._phraseLen, self._vecLen), dtype=np.float)

            print("      train")
            for i,l in enumerate(textTrain):
                if i%1000==0:
                    print("         processed line {}/{}           ".format(i,len(textTrain)), end='\r')
            ...
            for i,l in enumerate(textTest):
                if i%1000==0:
                    print("         processed line {}/{}           ".format(i,len(textTest)), end='\r')
    ...
    def _createModelCat(self, outDim):
        ...
        model.add(LSTM(self._hiddenNodes, input_shape=(self._phraseLen, self._vecLen), dropout=self._dropout, recurrent_dropout=self._dropout))
    ...
    def _evaluate(self, X, y):
        ...
        for task in self._tasks:
            table.append([" ", " ", " ", " ", " ", " ", " ", " "])
            print(task)
            yp[task] = self.model[task].predict_proba(X[task])
            yt[task] = y[task]
