...
class MyLSTM:
    ...
    #_tasks = ['sede1']
    _filesFolder = "./files9noGLOVE"
    _memmapFolder = "./memmapLSTMnoGLOVE"
    ...
    def extractData(self):
        ...
        vectorizer = CountVectorizer(min_df=3, max_df=0.3, max_features=900, strip_accents='unicode')
        #vectorizer = CountVectorizer(min_df=3, max_df=0.3, strip_accents='unicode')
        vectorizer.fit(text)
        ...
        for i,l in enumerate(text):
            if i%1000==0:
                print("processed line {}/{}          ".format(i,len(text)), end="\r")
            X.append(sp.sparse.lil_matrix(np.zeros(shape=(self._phraseLen, self._vecLen), dtype=np.int)))
            j = 0
            for w in l.split():
                bw = vectorizer.transform([w])[0]
                if np.sum(bw) == 1:
                    X[i][j] = bw
                    j += 1
        ...
        self.indexTrain, self.indexTest = train_test_split(np.arange(len(X)), test_size=0.2, random_state=42)
        ...
        for i,j in enumerate(self.indexTrain):
            if i%1000==0:
                print("splitted train line {}/{}        ".format(i,len(self.indexTrain)), end="\r")
            self.XTrain[i] = X[j].todense()
        print("")
        for i,j in enumerate(self.indexTest):
            if i%1000==0:
                print("splitted test line {}/{}         ".format(i,len(self.indexTest)), end="\r")
            self.XTest[i] = X[j].todense()
        print("")
        del X
        self.XTrain.flush()
        self.XTest.flush()
