...
class MyLSTM:
    _filesFolder = "./filesFolds-LSTMcomplete"
    _memmapFolder = "./memmapFolds-LSTMcomplete"
    _tmpFolder = "./tmpFolds-LSTMcomplete"
    ...
    def __init__(self, fold=0, tasks=['sede1', 'sede12', 'morfo1', 'morfo2'], corpusFolder="corpusLSTM_ICDO3", foldsFolder="folds10", fileVectors="vectors.txt", hiddenNodes=200, dropout=0.3, patience=2, batchSize=32, epochs=100):
        self._tasks = tasks
    ...
    def extractData(self):
        ...
        for task in self._tasks:
            ...
            for line in textTrain+textTest:
                validWords = 0
                for word in line.split():
                    if word in vectors:
                        validWords += 1
                if validWords > self._phraseLen:
                    self._phraseLen = validWords
        ...
        for task in self._tasks:
            ...
            self.XTrain[task] = np.memmap(self._fileMemmapXTrain[task], mode='w+', shape=(len(textTrain), self._phraseLen, self._vecLen), dtype=np.float32)
            self.XTest[task] = np.memmap(self._fileMemmapXTest[task], mode='w+', shape=(len(textTest), self._phraseLen, self._vecLen), dtype=np.float32)

            print("      train")
            textTrainLen = len(textTrain)
            for i,l in enumerate(textTrain):
                if i%1000==0:
                    print("         processed line {}/{}           ".format(i,textTrainLen), end='\r')

                j = 0
                for w in l.split():
                    try:
                        self.XTrain[task][i,j] = vectors[w]
                        j += 1
                    except KeyError:
                        pass
                #for r in range(j,self._phraseLen):
                #    self.XTrain[task][i,r] = np.zeros(self._vecLen)

            print("         processed line {}/{}           ".format(i+1,textTrainLen))
            self.XTrain[task].flush()

            print("      test")
            textTestLen = len(textTest)
            for i,l in enumerate(textTest):
                if i%1000==0:
                    print("         processed line {}/{}           ".format(i,textTestLen), end='\r')

                j = 0
                for w in l.split():
                    try:
                        self.XTest[task][i,j] = vectors[w]
                        j += 1
                    except KeyError:
                        pass
                #for r in range(j,self._phraseLen):
                #    self.XTest[task][i,r] = np.zeros(self._vecLen)

            print("         processed line {}/{}           ".format(i+1,textTestLen))
            self.XTest[task].flush()
            ...
            self.yTrain[task] = np.memmap(self._fileMemmapYTrain[task], mode='w+', shape=(len(yTrain), len(self.lb[task].classes_)), dtype=np.int32)
            self.yTest[task] = np.memmap(self._fileMemmapYTest[task], mode='w+', shape=(len(yTest), len(self.lb[task].classes_)), dtype=np.int32)
    ...
    def loadData(self):
        ...
        for task in self._tasks:
            self.XTrain[task] = np.memmap(self._fileMemmapXTrain[task], mode='r', shape=shapes[task]['XTrain'], dtype=np.float32)
            self.XTest[task] = np.memmap(self._fileMemmapXTest[task], mode='r', shape=shapes[task]['XTest'], dtype=np.float32)

            self.yTrain[task] = np.memmap(self._fileMemmapYTrain[task], mode='r', shape=shapes[task]['yTrain'], dtype=np.int32)
            self.yTest[task] = np.memmap(self._fileMemmapYTest[task], mode='r', shape=shapes[task]['yTest'], dtype=np.int32)
    ...
    def _createModelCat(self, outDim):
        ...
        model.add(LSTM(self._hiddenNodes, dropout=self._dropout, recurrent_dropout=self._dropout))
    ...
    def evaluate(self):
        ...
        for task in self._tasks:
            ...
            yc[task] = np.zeros(yt[task].shape, np.int32)
