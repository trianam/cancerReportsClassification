...
class MyLSTM:
    ...
    _textFileTrain = _corpusFolder+"/folds/sede12/0/textTrain.txt"
    _textFileTest = _corpusFolder+"/folds/sede12/0/textTest.txt"
    _fileYTrain = _corpusFolder+"/folds/sede12/0/yTrain.txt"
    _fileYTest = _corpusFolder+"/folds/sede12/0/yTest.txt"
    ...
    def extractData(self):
        ...
        self.XTrain = np.memmap(self._fileMemmapXTrain, mode='w+', shape=(len(textTrain), self._phraseLen, self._vecLen), dtype=np.float)
        self.XTest = np.memmap(self._fileMemmapXTest, mode='w+', shape=(len(textTest), self._phraseLen, self._vecLen), dtype=np.float)
        
        for i,l in enumerate(textTrain):
            if i%1000==0:
                print("processed line {}/{}".format(i,len(textTrain)))

            j = 0
            for w in l.split():
                if w in vectors:
                    self.XTrain[i,j] = vectors[w]
                    j += 1
        ...
        for i,l in enumerate(textTest):
            if i%1000==0:
                print("processed line {}/{}".format(i,len(textTest)))

            j = 0
            for w in l.split():
                if w in vectors:
                    self.XTest[i,j] = vectors[w]
                    j += 1
                    if j >= self._phraseLen:
                        break
