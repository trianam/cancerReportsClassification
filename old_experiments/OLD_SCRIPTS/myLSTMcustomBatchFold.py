...
class MyLSTM:
    _filesFolder = "./filesFolds-LSTMcustomBatchRegularized3"
    _memmapFolder = "./memmapFolds-LSTMcustomBatch"
    ...
    def __init__(self, fold=0, tasks=['sede1', 'sede12', 'morfo1', 'morfo2'], corpusFolder="corpusLSTM_ICDO3", foldsFolder="folds10", fileVectors="vectors.txt", phraseLen = 200, lstmCells=150, learningRate=0.005, learningRateDecay=0., dropout=0.5, regularization=0.001, patience=2, epochs=100):
        ...
        classesIndexesTrain = self._memmapFolder+"/"+str(fold)+"/classesIndexesTrain"
        if not os.path.exists(classesIndexesTrain):
            os.makedirs(classesIndexesTrain)
        self._fileClassesIndexesTrain = classesIndexesTrain+"/classesIndexesTrain.p"
    ...
    def _createClassesIndexesTrain(self, yTrain):
        classesIndexesTrain = {}

        classesIndexesTrain['indexes'] = defaultdict(list)
        for i,y in enumerate(yTrain):
            classesIndexesTrain['indexes'][y].append(i)

        classesIndexesTrain['lenAll'] = {}
        classesIndexesTrain['lenVal'] = {}
        for c in classesIndexesTrain['indexes'].keys():
            classesIndexesTrain['lenAll'][c] = len(classesIndexesTrain['indexes'][c])
            classesIndexesTrain['lenVal'][c] = math.ceil(classesIndexesTrain['lenAll'][c] * 0.2)

        return classesIndexesTrain
    ...
    def extractData(self):
        ...
        for task in self._tasks:
            ...
            print("      indexes")            
            self._classesIndexesTrain[task] = self._createClassesIndexesTrain(yTrain)
            
        pickle.dump(shapes, open(self._fileShapes, "wb"))
        pickle.dump(self._classesIndexesTrain, open(self._fileClassesIndexesTrain, "wb"))

    def loadData(self):
        self._classesIndexesTrain = pickle.load(open(self._fileClassesIndexesTrain, 'rb'))
        shapes = pickle.load(open(self._fileShapes, 'rb'))
    ...
    def _createModel(self, outDim):
        ...
        model.add(Bidirectional(LSTM(self._lstmCells, dropout=self._dropout, recurrent_dropout=self._dropout, return_sequences=True, kernel_regularizer=regularizers.l2(self._regularization)), input_shape=(self._phraseLen, self._vecLen)))
        model.add(Bidirectional(LSTM(self._lstmCells, dropout=self._dropout, recurrent_dropout=self._dropout, return_sequences=True, kernel_regularizer=regularizers.l2(self._regularization))))
        ...
        model.add(Dense(outDim, activation='relu', kernel_regularizer=regularizers.l2(self._regularization)))
        model.add(Dense(outDim, activation='softmax', kernel_regularizer=regularizers.l2(self._regularization)))
    ...
    _randintTrain = lambda self,task,c: randint(0, self._classesIndexesTrain[task]['lenAll'][c]-self._classesIndexesTrain[task]['lenVal'][c]-1)
    
    _randintVal = lambda self,task,c: randint(self._classesIndexesTrain[task]['lenAll'][c]-self._classesIndexesTrain[task]['lenVal'][c], self._classesIndexesTrain[task]['lenAll'][c]-1)

    def _batchGenerator(self, task, X, y, randFun):
        #TODO: this is good for sede12, check for other tasks (remember _calculateStep*)
        switch = 0
        partNum = 3
        allClasses = sorted(self._classesIndexesTrain[task]['indexes'].keys())

        while True:
            batchX = []
            batchY = []
            classes = allClasses[switch::partNum]
            switch = (switch+1) % partNum

            for c in classes:
                index = self._classesIndexesTrain[task]['indexes'][c][randFun(task,c)]
    ...
    _calculateStepTrain = lambda self,task: math.floor((len(self.yTrain[task])-math.ceil(len(self.yTrain[task]) * 0.2)) / (len(self._classesIndexesTrain[task]['indexes']) / 3.))
    _calculateStepVal = lambda self,task: math.floor(math.ceil(len(self.yTrain[task]) * 0.2) / (len(self._classesIndexesTrain[task]['indexes']) / 3.))
    ...
    def createModels(self):
        ...
        for task in self._tasks:
            ...
            self.history[task] = [self.model[task][0].fit_generator(
                self._batchGenerator(task, self.XTrain[task], self.yTrain[task], self._randintTrain),
                self._calculateStepTrain(task),
                validation_data = self._batchGenerator(task, self.XTrain[task], self.yTrain[task], self._randintVal),
                validation_steps = self._calculateStepVal(task),
                epochs = self._epochs, 
    ...
    def continueTrainModels(self, fromModel=-1, newLearningRate=None, useEarlyStopping=False, epochs=None):
        ...
        for task in self._tasks:
            ...
            self.history[task].append(self.model[task][-1].fit_generator(
                self._batchGenerator(task, self.XTrain[task], self.yTrain[task], self._randintTrain),
                self._calculateStepTrain(task),
                validation_data = self._batchGenerator(task, self.XTrain[task], self.yTrain[task], self._randintVal),
                validation_steps = self._calculateStepVal(task),
    ...
    def evaluate(self):
        ...
        for task in self._tasks:
            ...
            for modelIndex, currModel in enumerate(self.model[task]):
                if not os.path.isfile(self._calculateFileEvaluation(task, modelIndex)) or not os.path.isfile(self._calculateFileTable(task, modelIndex)):
