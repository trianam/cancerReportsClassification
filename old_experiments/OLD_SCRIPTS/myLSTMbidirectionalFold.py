...
class MyLSTM:
    _filesFolder = "./filesFolds-LSTMbidirectional"
    _memmapFolder = "./memmapFolds-LSTMbidirectional"
    _tmpFolder = "./tmpFolds-LSTMbidirectional"
    ...
    def __init__(self, fold=0, tasks=['sede1', 'sede12', 'morfo1', 'morfo2'], corpusFolder="corpusLSTM_ICDO3", foldsFolder="folds10", fileVectors="vectors.txt", phraseLen = 100, lstmCells=10, hiddenNodes=400, dropout=0.3, patience=2, batchSize=65, epochs=100):
        self._tasks = tasks
    ...
    def _processText(self, text, fileMemmap, vectors):
        X = np.memmap(fileMemmap, mode='w+', shape=(2, len(text), self._phraseLen, self._vecLen), dtype=np.float)
        ...
        for iDoc,l in enumerate(text):
            ...
            for iWord in range(len(words)):
                try:
                    X[0,iDoc,iWordLeft] = vectors[words[iWord]]
                    iWordLeft += 1
                except IndexError:
                    break
                except KeyError:
            ...
            #right
            iWordRight = 0
            for iWord in reversed(range(len(words))):
                try:
                    X[1,iDoc,iWordRight] = vectors[words[iWord]]
                    iWordRight += 1
                except IndexError:
                    break
                except KeyError:
    ...
    def createModels(self):
        ...
        for task in self._tasks:
            ...
            self.history[task] = self.model[task].fit([self.XTrain[task][0], self.XTrain[task][1]], self.yTrain[task], validation_split=0.2, epochs=epochs, batch_size=batchSize, callbacks=[early_stopping])
    ...
    def evaluate(self):
        ...
        for task in self._tasks:
            table.append([" ", " ", " ", " ", " ", " ", " ", " "])
            print(task)
            yp[task] = self.model[task].predict_proba([self.XTest[task][0], self.XTest[task][1]])
