...
class MyLSTM:
    _filesFolder = "./filesFolds-LSTMbidirectional5eNoNegation"
    _memmapFolder = "./memmapFolds-LSTMbidirectionalNoNegation"
    _negations = ["no", "non", "né", "nè", "neppure", "neanche", "nemmeno", "affatto", "alcuno", "nessuno", "nulla", "niente", "assolutamente", "mica"]
    
    def __init__(self, fold=0, tasks=['sede1', 'sede12', 'morfo1', 'morfo2'], corpusFolder="corpusLSTM_ICDO3", foldsFolder="folds10", fileVectors="vectors.txt", phraseLen = 200, lstmCells=150, learningRate=0.001, learningRateDecay=0., dropout=0.5, patience=2, batchSize=65, epochs=100, removeNegations=True):
        self._tasks = tasks
        ...
        self._removeNegations = removeNegations
    ...
    def _processText(self, text, fileMemmap, vectors):
        ...
        for iDoc,l in enumerate(text):
            ...
            words = l.split()
            if self._removeNegations:
                words = [ w for w in words if w not in self._negations ]
            ...
            for iWord in range(len(words)):
                try:
                    w = words[iWord]
                    if (not self._removeNegations) or (w not in self._negations):
                        X[iDoc,iWordValid] = vectors[w]
                        iWordValid += 1
                    iWordWithNeg += 1
                    if iWordWithNeg >= self._phraseLen:
                        break
                except IndexError:
                    break
                except KeyError:
    ...
    def _createModel(self, outDim):
        ...
        model.add(Bidirectional(LSTM(self._lstmCells, dropout=self._dropout, recurrent_dropout=self._dropout, return_sequences=True), input_shape=(self._phraseLen, self._vecLen)))
