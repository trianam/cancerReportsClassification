...
outDir = "./corpusMMI"
fileInCorpus = outDir+"/corpusFilteredBalanced.p"
#fileOutHdf5 = outDir+"/corpusProcessed.hdf5"
fileOutX = outDir+"/X.npy"
fileOutY = outDir+"/y.npy"
...
y = np.zeros(shape=(len(corpus['text'])), dtype=np.float)
...
for iDoc,l in enumerate(corpus['text']):
    ...
    if corpus[balanceKey][iDoc] == values[1]:
        y[iDoc] = 1.
